...
--kein Move von DGruppe zu einer anderen
...
select * from ptab where id = 117 --20000 Seiten --Table Scan
select * from ptab where nummer = 117---100 Seiten -- Table Scan (Heap)
...
alter partition function fzahl() split range (5000)--jetzt!!
...
--nur die F()
alter partition function fzahl() merge range(100)--mehr braucht man nicht
...
Meldung Meldung 4972, Ebene 16, Status 1, Zeile 293
Fehler bei der ALTER TABLE SWITCH-Anweisung. CHECK-Einschränkungen 
oder die Partitionsfunktion der dbdemo.dbo.archiv-Quelltabelle lassen Werte zu, 
die von CHECK-Einschränkungen oder der Partitionsfunktion in der dbdemo.dbo.ptab-Zieltabelle nicht zugelassen werden., Ebene 16, Status 1, Zeile 293
Fehler bei der ALTER TABLE SWITCH-Anweisung. CHECK-Einschränkungen 
oder die Partitionsfunktion der dbdemo.dbo.archiv-Quelltabelle lassen Werte zu, 
die von CHECK-Einschränkungen oder der Partitionsfunktion in der dbdemo.dbo.ptab-Zieltabelle nicht zugelassen werden.

alter table archiv add CONSTRAINT CHECK201 Check(nummer>=200)

alter table archiv switch to ptab partition 3

ALTER TABLE dbo.archiv
	DROP CONSTRAINT CHECK201
GO
ALTER TABLE dbo.archiv ADD CONSTRAINT
	CHECK201 CHECK (([nummer]>=(201) and nummer <=20000 and nummer is not null))


select min(nummer), max(nummer) from archiv
